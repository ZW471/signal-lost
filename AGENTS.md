...
Signal Lost is a cyberpunk knowledge-roguelike text RPG. The game engine is a LangGraph state machine with a browser-based GUI (FastAPI + WebSocket). It supports multiple LLM providers (Anthropic, OpenAI, OpenRouter, LM Studio, Codex CLI, Codex CLI) and is bilingual (English/中文).
...
│   ├── scripts/           # Headless engine + Codex/Codex CLI wrappers
...
| `tests/scripts/claude_llm.py` | Codex CLI LLM wrapper (BaseChatModel) |
